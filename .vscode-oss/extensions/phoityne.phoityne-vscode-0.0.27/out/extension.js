...
let logger = vscode.window.createOutputChannel("haskell-debug-adapter");
...
//
//
function startHaskellDebuggingFromEditor(fileUri) {
...
//
//
function startHaskellDebuggingFromExplore(fileUri) {
...
//
//
function runDebugger(fileUri, startFile, funcName, args) {
    const confName = 'haskell-debug-adapter';
    let launch = vscode.workspace.getConfiguration('launch', fileUri);
    let confs = launch.get('configurations');
    if (!confs) {
...
//
//
function isNeedArgs(editor, funcName) {
    const selection = editor.selection;
